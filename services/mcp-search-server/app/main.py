"""MCP Search Server - Main FastAPI Application."""
...
async def lifespan(app: FastAPI):
    ...
    global search_client, feed_client, research_client, mcp_handler
    ...
    logger.info("MCP Search Server started successfully")
    tools_response = await mcp_handler.list_tools()
    logger.info(f"Registered {len(tools_response.tools)} MCP tools")

    yield

    # Cleanup
    logger.info("Shutting down MCP Search Server...")
    await search_client.close()
    await feed_client.close()
    await research_client.close()
    await cache_manager.close()
    logger.info("MCP Search Server shutdown complete")


# Create FastAPI app
app = FastAPI(
    title="MCP Search Server",
    description="Model Context Protocol gateway for Search Services",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root():
    ...
    return {
        ...
        "backend_services": {
            ...
            "osint-service": settings.osint_url,
