"""MCP Content Server - FastAPI Application.

Provides MCP (Model Context Protocol) gateway for:
- Feed Service (8101): RSS feeds, items, quality, scheduling
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        f"MCP Content Server ready with {len(tool_registry.tools)} tools"
    )

    yield

    # Shutdown
    logger.info("Shutting down MCP Content Server...")
...
# Create FastAPI app
app = FastAPI(
    title="MCP Content Server",
    description="MCP Gateway for Feed Service",
    version=settings.server_version,
    lifespan=lifespan,
)
