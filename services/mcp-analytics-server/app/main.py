"""MCP Analytics Server - Main FastAPI Application."""
...
execution_client: ExecutionClient = None
...
async def lifespan(app: FastAPI):
    ...
    global analytics_client, prediction_client, execution_client, mcp_handler
    ...
    logger.info("MCP Analytics Server started successfully")
    ...
    # Cleanup
    logger.info("Shutting down MCP Analytics Server...")
    await analytics_client.close()
    await prediction_client.close()
    await execution_client.close()
    await cache_manager.close()
    logger.info("MCP Analytics Server shutdown complete")


# Create FastAPI app
app = FastAPI(
    title="MCP Analytics Server",
    description="Model Context Protocol gateway for Analytics Services",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root():
    ...
    return {
        ...
        "backend_services": {
            ...
            "prediction-service": settings.prediction_url,
            "execution-service": settings.execution_url,
        },
    }
