"""MCP Orchestration Server - FastAPI Application.

Provides MCP (Model Context Protocol) gateway for:
- Scheduler Service (8108): Jobs, cron, feed scheduling
- MediaStack Service (8121): Live/historical news, sources
- Scraping Service (8115): Web scraping, DLQ, cache, proxy, queue, Wikipedia
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        f"MCP Orchestration Server ready with {len(tool_registry.tools)} tools"
    )

    yield

    # Shutdown
    logger.info("Shutting down MCP Orchestration Server...")
...
# Create FastAPI app
app = FastAPI(
    title="MCP Orchestration Server",
    description="MCP Gateway for Scheduler Service",
    version=settings.server_version,
    lifespan=lifespan,
)
...
async def get_info():
    ...
    return {
        ...
        "backend_services": {
            ...
            "mediastack": settings.mediastack_service_url,
