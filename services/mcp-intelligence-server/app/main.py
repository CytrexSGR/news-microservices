"""MCP Intelligence Server - Main FastAPI Application."""
...
content_analysis_client: ContentAnalysisClient = None
entity_canon_client: EntityCanonClient = None
intelligence_client: IntelligenceClient = None
...
async def lifespan(app: FastAPI):
    ...
    global content_analysis_client, entity_canon_client, intelligence_client, narrative_client, mcp_handler

    logger.info("Starting MCP Intelligence Server...")
    ...
    logger.info("MCP Intelligence Server started successfully")

    yield

    # Cleanup
    logger.info("Shutting down MCP Intelligence Server...")
    ...
    logger.info("MCP Intelligence Server shutdown complete")


# Create FastAPI app
app = FastAPI(
    title="MCP Intelligence Server",
    description="Model Context Protocol gateway for Intelligence Services",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root():
    ...
    return {
        ...
        "backend_services": {
            "content-analysis-v3": settings.content_analysis_url,
            "entity-canonicalization": settings.entity_canon_url,
            "intelligence-service": settings.intelligence_url,
            "narrative-service": settings.narrative_url,
        },
    }
